...
**查詢功能 + Dashboard 已完成，寫入功能待開發。** Laravel 13 + Sanctum 已 scaffold、48 個 Blade Component 已全數實作、Chat-to-query（唯讀查詢）後端 18 支 API 全部到位（含 SSE 串流 + Dashboard）、Golden Test 150 筆 100% pass、Dashboard 獨立頁面已上線（月/季/年指標 + 趨勢）、聊天頁 Claude 風格空狀態 + markdown 渲染。下一步：Chat-to-operate（INSERT/UPDATE/DELETE + 確認流程 + 檔案上傳）。

**已建立的核心目錄：**
- `app/Http/Controllers/Api/` — AuthController, ChatController, StreamChatController, ChatHistoryController, DashboardController
- `app/Http/Controllers/Web/` — AuthPageController, ChatPageController, DashboardPageController
- `app/Services/` — Ai/（LlmGateway, OpenAiGateway, QueryEngine, SqlValidator, ConfidenceEstimator）, Schema/, Tenant/（TenantManager, TenantDatabaseManager）, Dashboard/（DashboardService）
...
- `app/DataTransferObjects/` — Chat/, Schema/, Dashboard/（DashboardMetric）
- `app/Enums/` — ChatResponseType, ConfidenceLevel, UserRole, ValueFormat, AggregationType
- `app/Support/` — CurrencyFormatter, NumberFormatter
- `app/Console/Commands/` — GoldenAccuracyCommand, TenantProvisionCommand
- `app/Providers/RepositoryServiceProvider.php`
- `database/migrations/tenant/` — 16 個 tenant-specific migrations（categories → schema_metadata + dashboard columns）

**尚未建立：** 寫入操作引擎（INSERT/UPDATE/DELETE + 確認流程）、檔案上傳模組。實作新功能前先 `Glob` 確認對應目錄是否存在。
...
# Demo 租戶 seed（帳號密碼皆為 admin@example.com）
php artisan db:seed --class=DemoSeeder

# 租戶 DB 佈建（含 migration + seed）
...
| UI 設計 | Spotify DESIGN.md（根目錄 `DESIGN.md`，深色唯一模式） |
...
- **Blade 元件化：** 所有 UI 用 `<x-chat.bubble>` 等巢狀命名空間的 Blade Component；聊天頁和 Dashboard 頁因 Alpine.js 動態渲染需求，大部分 UI 以 inline CSS class 渲染（非 Blade component 引用）
- **頁面路由：** `/` → redirect `/dashboard`、`/dashboard`（首頁）、`/chat`（聊天）、`/login`、`/forgot-password`、`/reset-password/{token}`
- **Dashboard 預定義查詢：** `DashboardService` 用 Laravel Query Builder 預定義查詢（非 LLM 生成），分 sales/finance/operations 三組，每組有月/季/年三個時間維度 + 趨勢百分比
- **AI 回覆 markdown 渲染：** 前端用 `marked.js`（`window.renderMarkdown`），AI 氣泡用 `x-html` + `.prose-ai` CSS
- **ValueFormat::format()：** 統一格式化入口（Currency → `NT$1,234,567`、Count → `1,234`），QueryEngine 和 DashboardService 共用
...
所有 48 個元件在 `resources/views/components/{namespace}/`。
...
新元件一律用 CSS class 和變數，**禁止 inline style**。

- **Heading：** `.h-hero` (32px) / `.h-section` (24px) / `.h-card` (18px, semibold) / `.h-sub` (16px, bold)
...
- **Confidence：** `.confidence-high/mid/low`、`.stat-trend-up/-down/-neutral`
- **Chat welcome：** `.chat-welcome`（Claude 風格居中）、`.chat-welcome-input`、`.chat-hint-chip`（快捷建議 pill）
- **Chat bottom：** `.chat-bottom-form`、`.chat-bottom-input`、`.chat-bottom-send`（對話中底部輸入框）
- **AI prose：** `.prose-ai`（markdown 渲染容器，覆蓋 Tailwind reset 的 list-style）
- **Dashboard：** `.dashboard-grid`、`.dashboard-stat-card.is-currency/.is-count`、`.dash-period-select`、`.dashboard-section-title`、`.dash-content`
- **Sidebar nav：** `.sidebar-nav`、`.sidebar-nav-item.is-active`（Dashboard / 聊天頁面切換）
- **Alpine：** 需要隱藏的元件加 `x-cloak`（全域規則已定義）

CSS 變數體系：`--bg-page/-card/-white/-sand/-cream`, `--text-primary/-secondary/-tertiary`, `--brand/-hover/-active/-rgb`, `--border`, `--shadow-card/-modal/-inset`, `--ring-focus/-error/-brand`（`--ring-default` 為 `none`，深色靠色差分層）。完整清單見 `app.css` 的 `:root` 區塊。字型只有 `--font-sans` 和 `--font-mono`，無 serif。
...
### 規格書
- [00 元件庫](docs/spec/00-component-library.md) — Blade Component 定義（核心 + inline 渲染說明 + 暫不使用）
- [01 後端](docs/spec/01-phase1-backend.md) — Chat-to-Operate API（查詢 + 寫入 + 上傳 + Dashboard 預定義查詢）
- [02 前端](docs/spec/02-phase1-frontend.md) — Dashboard 獨立頁面 + 聊天頁（Claude 風格 + markdown）+ 寫入確認 + 檔案上傳

## Git Submodules

- `awesome-design-md/` — [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md) 的 DESIGN.md 集合（參考用，目前使用 Spotify 風格，見根目錄 `DESIGN.md`）
